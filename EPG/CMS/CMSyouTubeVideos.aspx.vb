...
Public Class CMSyouTubeVideos
    ...
    Protected Sub btnUpdate_Click(sender As Object, e As EventArgs) Handles btnUpdate.Click
        If ddlProgramme.SelectedIndex = 0 Then
            myMessageBox("Please select programme first!")
        Else
    ...
    Protected Sub bindGrid(ByVal paging As Boolean, boolFromSearch As Boolean)
        ...
        If boolFromSearch Then
            If ddlChannelSearch.SelectedIndex = 0 Then
                sqlDSYouTubeVideos.SelectCommand = "select c.channelid,b.ProgName,a.rowid,a.duration, b.channelid,a.progid,a.season,a.keywords,a.synopsis,a.likes,a.YouTubeChannelId,a.VideoTitle ," & _
                    ...
                    " from youtubevideos a join mst_youtubechannels c on a.youtubechannelid=c.youtubechannelid left outer join mst_program b on a.progid=b.progid where a.videotitle like '%" & txtSearch.Text.Trim & "%' and " & _
            ...
            Else
                sqlDSYouTubeVideos.SelectCommand = "select c.channelid,b.ProgName,a.rowid,a.duration,b.channelid,a.progid,a.season,a.keywords,a.synopsis,a.likes,a.YouTubeChannelId,a.VideoTitle ," & _
                    "REPLACE(a.videourl,'https://www.youtube.com/watch?v=','') VideoURL,a.videourl VideoURL1,a.verified, a.episodeId ,convert(varchar,a.publishdate,106) publishdate,a.modifiedby,a.modifiedat" & _
                    " from youtubevideos a join mst_youtubechannels c on a.youtubechannelid=c.youtubechannelid left outer join mst_program b on a.progid=b.progid where a.videotitle like '%" & txtSearch.Text.Trim & "%' and " & _
                    " a.youtubechannelid='" & ddlChannelSearch.SelectedValue & "' and convert(varchar,publishdate,112) between '" & Convert.ToDateTime(txtStartDate.Text).ToString("yyyyMMdd") & "' and '" & Convert.ToDateTime(txtEndDate.Text).ToString("yyyyMMdd") & "' " & _
                  " and verified='" & chkVerifiedSearch.Checked & "'"
            End If

        Else
            sqlDSYouTubeVideos.SelectCommand = "select  c.channelid,b.ProgName,a.rowid,a.duration,b.channelid,a.progid,a.season,a.keywords,a.synopsis,a.likes,a.YouTubeChannelId,a.VideoTitle ,REPLACE(a.videourl,'https://www.youtube.com/watch?v=','') VideoURL,a.videourl VideoURL1,a.verified, a.episodeId ,convert(varchar,a.publishdate,106) publishdate,a.modifiedby,a.modifiedat from youtubevideos a join mst_youtubechannels c on a.youtubechannelid=c.youtubechannelid  left outer join mst_program b on a.progid=b.progid where a.videotitle like '%" & txtSearch.Text.Trim & "%' and convert(varchar,a.publishdate,112) between convert(varchar,dbo.getlocaldate()-30,112) and convert(varchar,dbo.getlocaldate(),112) "
        End If
