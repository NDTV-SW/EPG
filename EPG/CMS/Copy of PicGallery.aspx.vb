...
Public Class PicGallery
    ...
    Protected Sub OnUploadComplete(sender As Object, e As AjaxFileUploadEventArgs)
        Try
            ...
            Dim adpGetMaxFile As New SqlDataAdapter("select count(*) from picgallery_pictures where galleryId='" & ddlGallery.SelectedValue & "'", ConString)
            Dim dtGetMaxFile As New DataTable
            adpGetMaxFile.Fill(dtGetMaxFile)

            Dim fileName As String = ddlGallery.SelectedValue & "_" & Convert.ToInt32(dtGetMaxFile.Rows(0)(0).ToString) + 1 & ".jpg"
            ...
            Dim adp As New SqlDataAdapter("insert into picgallery_pictures(galleryId,url,dateCreated,DateModified,cretedBy,ModifiedBy) " _
                                          & "values('" & ddlGallery.SelectedValue & "','" & StrFileSavePath & "',getdate(),getdate(),'" & User.Identity.Name & "','" & User.Identity.Name & "')", ConString)
            Dim dt As New DataTable
            adp.Fill(dt)
            grdPicGallery.DataBind()
            'Session("FileName") = "0"
            'Session("Profile") = "0"
            'Response.Redirect("cmsTVStarMaster.aspx")
            'End If
