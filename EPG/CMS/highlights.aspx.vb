...
Public Class highlights
    ...
    Private Sub bindGrid(ByVal paging As Boolean)
        ...
        Dim dt As DataTable = obj.executeSQL("select * from api_fn_get_spuul_highlights_filtered_bulk (232) order by category,progdate", False)
    ...
    Protected Sub btnHighLight_Click(sender As Object, e As EventArgs) Handles btnHighLight.Click
        ...
        For Each row In grdHighLights.Rows
            ...
            If chkHighLight.Checked Then
                ...
                Dim VADate As String = TryCast(row.FindControl("lbISTDate"), Label).Text
                Dim VATime As String = TryCast(row.FindControl("lbISTTime"), Label).Text
                ...
                Dim FreshAirTimeVA As String = TryCast(row.FindControl("lbISTDate"), Label).Text
                Dim EpisodeNo As String = TryCast(row.FindControl("lbEpisodeNo"), Label).Text
                Dim subgenre As String = TryCast(row.FindControl("lbSubGenre"), Label).Text
                Dim releaseyear As String = TryCast(row.FindControl("lbReleaseYear"), Label).Text
                Dim duration As String = TryCast(row.FindControl("lbDuration"), Label).Text

                ',, ,
                Dim obj As New clsExecute
                obj.executeSQL("insert into mst_highlights values('" & ChannelId & "','" & ProgName & "','" & Synopsis & "','" & StarCast & "'," & _
                               "'" & ISTDate & "','" & ISTTime & "','" & Genre & "','" & poster & "','" & VADate & "','" & VATime & "',1,'" & progType & _
                               "','" & freshRepeat & "','" & FreshAirTimeVA & "','" & EpisodeNo & "','" & subgenre & "','" & releaseyear & "','" & duration & "')", False)
    ...
    Private Sub ExportExcel()
        ...
        Dim obj As New clsExecute
        Dim dt As DataTable = obj.executeSQL("select distinct Channelid, ProgName,ProgType,istdate [Date], isttime [Time], Synopsis, Genre, Poster from vw_highlights_va order by istdate,isttime,channelid", False)
