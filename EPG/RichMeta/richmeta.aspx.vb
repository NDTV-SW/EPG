Public Class richmeta
    Inherits System.Web.UI.Page

    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Page.IsPostBack = False Then
            Dim li As HtmlGenericControl = TryCast(Master.FindControl("lirichmeta"), HtmlGenericControl)
    ...
    Protected Sub btnAdd_Click(sender As Object, e As EventArgs) Handles btnAdd.Click
        Try
            ...
            Dim strParams As String = "mode~id~type~name~displayname~originallanguageid~dubbedlanguge~parentalguide~rating~seasonno~seasonname~synopsis~longsynopsis~starcast~director~producer~writer~trivia~awards~releaseyear~country~genre~subgenre~verified~broadcasterid~team1~team2~series~sporttype~venue~showhost~active~updatedby~tags~genrename1~genrename2"
            Dim strType As String = "varchar~int~varchar~varchar~varchar~int~int~varchar~float~int~varchar~varchar~varchar~varchar~varchar~varchar~varchar~varchar~varchar~int~varchar~int~int~bit~int~varchar~varchar~varchar~varchar~varchar~varchar~bit~varchar~varchar~varchar~varchar"
            ...
            Else
                strVals = "U~" & lbId.Text & "~"
            End If
            strVals = strVals & ddlType.SelectedValue & "~" & txtName.Text & "~" & txtDisplayName.Text & "~" & ddlOriginalLanguage.SelectedValue & "~" & ddlDubbedLanguage.SelectedValue & "~"
            ...
            strVals = strVals & ddlSportsType.SelectedValue & "~" & txtVenue.Text & "~" & txtShowHost.Text & "~" & chkActive.Checked & "~" & User.Identity.Name & "~" & txtTags.Text & "~" & txtGenre1.Text & "~" & txtGenre2.Text
    ...
    Private Sub grdChannelmaster_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles grd1.SelectedIndexChanged
        Try
            ...
            If dt.Rows.Count = 1 Then
                ...
                txtGenre1.Text = dt.Rows(0)("genrename1").ToString.Trim
                txtGenre2.Text = dt.Rows(0)("genrename2").ToString.Trim
    ...
    Protected Sub grd1_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles grd1.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            Dim hyName As HyperLink = DirectCast(e.Row.FindControl("hyName"), HyperLink)
            Dim lbLabel As Label = DirectCast(e.Row.FindControl("lbID"), Label)
            hyName.NavigateUrl = "javascript:openWin('" & lbLabel.Text & "')"
    ...
    Private Sub bindControls()
        ddlBroadcaster.Enabled = False
        ddlSportsType.Enabled = False
        txtSeries.Enabled = False
        txtTeam1.Enabled = False
        txtTeam2.Enabled = False
        txtVenue.Enabled = False
        txtShowHost.Enabled = False

        If ddlType.SelectedValue = "Movie" Then
        ElseIf ddlType.SelectedValue = "Show" Then
            ddlBroadcaster.Enabled = True
        ElseIf ddlType.SelectedValue = "News" Then
            ddlBroadcaster.Enabled = True
            txtShowHost.Enabled = True
        ElseIf ddlType.SelectedValue = "Sports" Then
            ddlBroadcaster.Enabled = True
            ddlSportsType.Enabled = True
