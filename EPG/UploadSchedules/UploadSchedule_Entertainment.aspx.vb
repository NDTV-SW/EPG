...
Public Class UploadSchedule_Entertainment
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Catch ex As Exception
            Logger.LogError("Upload Schedule Entertainment", "Page Load", ex.Message.ToString, User.Identity.Name)
        End Try
    End Sub
    ...
    Private Sub btnUpload_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnUpload.Click
        ...
        Catch ex As Exception
            ...
            Logger.LogError("Upload Schedule Entertainment", "btnUpload_Click", ex.Message.ToString, User.Identity.Name)
        End Try
    End Sub
    ...
    Private Sub uploadChannelSchedule(ByVal channelid As String, ByVal filePath As String)
        ...
        Try
            ...
            If dt.Rows.Count > 0 Then
                For i = 0 To dt.Rows.Count - 1
                    Dim vChannelID As String, vProgName As String, vProgShortTitle As String, vNextProgName As String, vGenre As String, vSubGenre As String, vDate As String, vNextDate As String
                    Dim vTime As String, vDuration As Integer, vNextDuration As Integer, vDescription As String, vShowDescription As String, vEpisodeNo As String, vNextEpisodeNo As String
                    Dim vOriginalRepeat As String
                    ...
                    Dim vSeason As String, vEpisodeTitle As String
                    ...
                    vProgShortTitle = dt.Rows(i)(7).ToString
                    vEpisodeTitle = dt.Rows(i)(8).ToString
                    vEpisodeNo = dt.Rows(i)(9).ToString
                    vSeason = dt.Rows(i)(12).ToString
                    If vEpisodeNo = "" Then
                        vEpisodeNo = 0
                    End If

                    Try
                        vOriginalRepeat = dt.Rows(i)(10).ToString
                    Catch ex As Exception
                        vOriginalRepeat = ""
                    End Try
                    vDescription = dt.Rows(i)(13).ToString
                    vShowDescription = dt.Rows(i)(14).ToString
                    vOrigLang = dt.Rows(i)(15).ToString
                    vDubbedLang = dt.Rows(i)(16).ToString
                    vStartCast = dt.Rows(i)(17).ToString
                    vDirector = dt.Rows(i)(18).ToString
                    vProducer = dt.Rows(i)(19).ToString
                    vReleaseYear = dt.Rows(i)(20).ToString
                    vRating = dt.Rows(i)(21).ToString
                    vOriginCountry = dt.Rows(i)(22).ToString
                    If vOriginalRepeat.Trim = "P" Or vOriginalRepeat.Trim = "T" Then
                    ...
                    If i < dt.Rows.Count - 1 And vProgName.Trim <> "" Then
                        Try
                            ...
                            vNextEpisodeNo = dt.Rows(i + 1)(9).ToString
                    ...
                    If i = dt.Rows.Count - 1 Or vNextProgName.Trim = "" Then
                        Try
                            InsertEPGData(vChannelID, vProgName, vProgShortTitle, vGenre, vSubGenre, vDate, vTime, vDuration, vDescription, vShowDescription, vEpisodeNo, vOriginalRepeat, vOrigLang, vDubbedLang, vStartCast, vDirector, vProducer, vReleaseYear, vRating, vOriginCountry, vSeason, vEpisodeTitle, True, chkIgnoreSpecial.Checked)
                            bindAll()
                            Dim dtIsGenericSynopsis As DataTable = obj.executeSQL("select isnull(IsGenericSynopsis,0) from mst_channel where channelid='" & vChannelID & "'", False)
                            Dim boolIsGenericSynopsis As Boolean = dtIsGenericSynopsis.Rows(0)(0).ToString
                            If Not boolIsGenericSynopsis Then
                                If chkGeneric.Checked = False Then
                            ...
                            Exit For
                        Catch ex As Exception
                            Logger.LogError("Upload Schedule Entertainment", "uploadChannelSchedule", channelid & " || " & filePath & "||" & ex.Message.ToString, User.Identity.Name)
                        End Try

                    Else
                        InsertEPGData(vChannelID, vProgName, vProgShortTitle, vGenre, vSubGenre, vDate, vTime, vDuration, vDescription, vShowDescription, vEpisodeNo, vOriginalRepeat, vOrigLang, vDubbedLang, vStartCast, vDirector, vProducer, vReleaseYear, vRating, vOriginCountry, vSeason, vEpisodeTitle, False, chkIgnoreSpecial.Checked)
                    End If
        ...
        Catch ex As Exception
            ...
            Logger.LogError("Upload Schedule Entertainment", "UploadChannelSchedule", "Row:" & i & ", " & ex.Message.ToString, User.Identity.Name)
        End Try
    ...
    Private Sub InsertEPGData(vChannelName As String, vProgName As String, vProgShortTitle As String, vGenre As String, vSubGenre As String, vDate As Date, vtime As Date, vduration As Integer, VDescription As String, vShowDescription As String, ByVal vEpisodeNo As String, ByVal vOriginalRepeat As String, vOrigLang As String, vDubbedLang As String, vStartCast As String, vDirector As String, vProducer As String, vReleaseYear As String, vRating As String, vOriginCountry As String, vSeason As String, vEpisodeTitle As String, ByVal boolInsert As Boolean, vIgnoreSpecial As Boolean)
        If Not vIgnoreSpecial Then
            ...
            vShowDescription = Logger.RemSplCharsEng(vShowDescription).Trim
        End If
        If VDescription.Length > 1000 Then
            VDescription = VDescription.Substring(0, 995)
        End If
        If vShowDescription.Length > 1000 Then
            vShowDescription = vShowDescription.Substring(0, 995)
        End If
        ...
        If Not (vduration = 0) Then
            strSql = strSql & "insert into map_epgexcel(channelID, [Program Name],eshorttitle,Genre,broadgenre,subgenre,Date,Time,Duration, Description,[show-wise description], [Episode No],liverepeat,origlang,dubbedlang,actor,director,producer,releaseyear,pg,origincountry,season,episodictitle) "
            strSql = strSql & "values('" & vChannelName & "','" & vProgName.Trim.Replace("'", "''") & "','" & vProgShortTitle.Trim.Replace("'", "''") & "','" & vGenre.Trim & "','" & vGenre.Trim & "','" & vSubGenre.Trim & "'"
            strSql = strSql & ",'" & vDate & "','" & vtime & "','" & vduration & "','" & VDescription.Replace("'", "''").Trim & "','" & vShowDescription.Replace("'", "''").Trim & "'"
            strSql = strSql & ",'" & vEpisodeNo.Trim & "','" & vOriginalRepeat & "','" & vOrigLang & "','" & vDubbedLang & "','" & vStartCast.Replace("'", "''") & "','" & vDirector.Replace("'", "''") & "','" & vProducer.Replace("'", "''") & "'"
            strSql = strSql & ",'" & vReleaseYear & "','" & vRating & "','" & vOriginCountry & "','" & vSeason.Replace("'", "''").Trim & "','" & vEpisodeTitle.Replace("'", "''").Trim & "');"
        End If
    ...
    Private Sub grdEpiMissing_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles grdEpiMissing.RowCommand
        ...
        Catch ex As Exception
            Logger.LogError("Upload Schedule Entertainment", e.CommandName.ToLower & " grdData_RowCommand", ex.Message.ToString, User.Identity.Name)
    ...
    Private Sub grdData_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles grdData.RowDataBound
        Try
            ...
            End If
        Catch ex As Exception
            Logger.LogError("Upload Schedule Entertainment", "grdData_RowDataBound", ex.Message.ToString, User.Identity.Name)
        End Try
    End Sub
    ...
    Private Sub btnBuildEPG_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnBuildEPG.Click
        ...
        Catch ex As Exception
            Logger.LogError("Upload Schedule Entertainment", "btnBuildEPG_Click", ex.Message.ToString, User.Identity.Name)
        End Try
    End Sub
    ...
    Private Sub grdGenre_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles grdGenre.RowCommand
        Try
            ...
            End If
        Catch ex As Exception
            Logger.LogError("Upload Schedule Entertainment", "grdGenre_RowCommand", ex.Message.ToString, User.Identity.Name)
        End Try
    End Sub
    ...
    Private Sub grdGenre_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles grdGenre.RowDataBound
        ...
        Catch ex As Exception
            Logger.LogError("Upload Schedule Entertainment", "grdGenre_RowDataBound", ex.Message.ToString, User.Identity.Name)
        End Try
    End Sub
    ...
    Private Sub btnUpdateGenre_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnUpdateGenre.Click
        Try
            ...
            bindAll()
        Catch ex As Exception
            Logger.LogError("Upload Schedule Entertainment", "btnUpdateGenre_Click", ex.Message.ToString, User.Identity.Name)
        End Try
    End Sub
    ...
    Protected Sub btnUpdateRich_Click(sender As Object, e As EventArgs) Handles btnUpdateRich.Click
        ...
        For Each row In grdMissingRichMeta.Rows
            ...
            If chkAddRich.Checked Then
                Dim sql As String
                sql = "insert into richmeta(type,broadcasterid,name,synopsis,genrename1,genrename2,parentalguide,releaseyear,starcast,director,producer,originallanguageid,dubbedlanguge,country,imageurl,imageurlportrait,seasonname) "
                sql = sql & "select '" & ddlType.SelectedValue & "' type,(select companyid from mst_channel where channelid=b.channelid) broadcasterid,[program name] progname,description,genre,subgenre,pg,releaseyear,actor,director,producer,isnull((select languageid from mst_language where fullname=a.origlang),2)"
                sql = sql & ",isnull((select languageid from mst_language where fullname=a.dubbedlang),2),origincountry,b.programlogo,b.programlogoportrait,a.season from map_epgexcel a "
