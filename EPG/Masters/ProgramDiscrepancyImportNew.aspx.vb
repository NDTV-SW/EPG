...
Public Class ProgramDiscrepancyImportNew
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            'Dim strLoggedInUser As String = User.Identity.Name.ToLower
            'If strLoggedInUser <> "rahulk" Or strLoggedInUser <> "krishank" Or strLoggedInUser <> "jeetendrag" Or strLoggedInUser <> "hemant" Or strLoggedInUser <> "kautilyar" Or strLoggedInUser <> "sachint" Then
    ...
    Private Sub bindMeGrid()
        Dim obj As New clsExecute
        Dim dt As DataTable = obj.executeSQL("sp_epg_import_program_central", "uploadedby~language", "varchar~int", User.Identity.Name & "~" & ddlLanguage.SelectedValue, True, False)
    ...
    Private Sub exec_ProcRegional(ByVal ProgId As Integer, ByVal ProgName As String, ByVal synopsis As String, ByVal LanguageID As Integer, ByVal ID As Integer, ByVal action As Char, ByVal vEpisodeNo As Integer)
        Try
            Dim obj As New clsExecute
            obj.executeSQL("sp_mst_ProgramRegional", "Rowid~ProgId~ProgName~Synopsis~EpisodeNo~LanguageId~Action~Actionuser",
                                       "Int~Int~nVarChar~nVarChar~Int~Int~Char~Varchar",
                                         "0~" & ProgId & "~" & Logger.RemSplCharsEng(ProgName) & "~" & Logger.RemSplCharsEng(synopsis) & "~" & vEpisodeNo & "~" & LanguageID.ToString.Trim & "~" & action.ToString.Trim & "~" & User.Identity.Name, True, False)
        Catch ex As Exception
    ...
    Private Sub ImportData(ByVal vchannelid As String, ByVal vProgName As String, ByVal vLanguageId As Integer, ByVal vProgramRegionalname As String, ByVal vsynopsis As String, ByVal vEpisode As String)
        ...
        Try
            ...
            Else
                Progname = vProgName.ToString.Trim
    ...
    Private Sub ExportwithTranslationExcel()
        ...
        If ddlType.SelectedValue = "Synopsis_HTRP" Or ddlType.SelectedValue = "Synopsis_LTRP" Or ddlType.SelectedValue = "Regional_HTRP" Or ddlType.SelectedValue = "Regional_LTRP" Then
