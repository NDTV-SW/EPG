...
Public Class ViewEPG
    ...
    Private Sub grdViewEPGBind()
        Try
            ...
            Dim myConnection As New SqlConnection(ConString)
            myConnection.Open()
            Dim adp As New SqlDataAdapter("select distinct a.channelid,Convert(varchar(11),a.progdate,113) as progdate1,DATENAME(weekday,a.progdate) as 'WeekDay',DATEPART( WW,a.progdate) as WeekNumber,a.progdate from mst_epg a join mst_Channel b on a.ChannelID = b.ChannelID join mst_program c on a.progid = c.progid where a.ChannelID='" & txtChannel.Text & "' and a.progdate between '" & txtStartDate.Text & "' and '" & txtEndDate.Text & "' order by progdate,3 asc", myConnection)
            Dim dt As New DataTable
            adp.Fill(dt)
    ...
    Private Sub exportExcel1()
        Try
            ...
            'Dim dt As DataTable = clsExecute.executeSQL("sp_export_exportepg_toexcel", "ChannelId~FromDate~ToDate", "VarChar~DateTime~DateTime", txtChannel.Text & "~" & Convert.ToDateTime(txtStartDate.Text.Trim) & "~" & Convert.ToDateTime(txtEndDate.Text.Trim), True)
            ...
            dscmd.SelectCommand.Parameters.Add(New SqlParameter("@ChannelId", SqlDbType.VarChar)).Value = txtChannel.Text.ToString.Trim
