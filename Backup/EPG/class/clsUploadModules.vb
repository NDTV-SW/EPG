...
Public Class clsUploadModules
        ...
        Try
            ...
            If vStatus.ToUpper <> "EPISODIC" Then
                ...
                obj.executeSQL("sp_mst_program", "ProgId~ChannelId~ProgName~GenreId~SubGenreId~RatingId~SeriesEnabled~ismovie~Active~Action~Actionuser", _
                                                               ...
                                                               "0~" & vChannelId & "~" & vProgName.Trim & "~" & vGenreId & "~" & 0 & "~" & vRatingId & "~" & IIf(vSeries.ToString.Trim = "1", True, False) & "~" _
                                                               & boolisMovie & "~" & IIf(vActive.ToString.Trim = "1", True, False) & "~A~" & user, True, False)
            End If
            Dim progid As String, strDesc As String
            progid = "0"
            strDesc = ""
            Dim sql As String = "select top 1 a.ProgId,b.Description  from mst_program a join map_EPGExcel b on dbo.FN_Sanitise(a.ProgName) =dbo.FN_Sanitise(b.[Program Name]) where dbo.FN_Sanitise(a.progname)='" & vProgName.Trim.Replace("'", "''") & "' and a.channelid='" & vChannelId.ToString.Trim & "'"
    ...
    Public Function sanitizeImageFile(ByVal strToSanitize As String) As String
        ...
        strToSanitize = strToSanitize.Replace("+", "_")
        strToSanitize = strToSanitize.Replace(",", "")
        strToSanitize = strToSanitize.Replace("à", "a")
        strToSanitize = strToSanitize.Replace("é", "e")
        strToSanitize = strToSanitize.Replace("ó", "o")
        strToSanitize = strToSanitize.Replace("í", "i")
        strToSanitize = strToSanitize.Replace("Å", "a")
        strToSanitize = strToSanitize.Replace("ö", "o")
        strToSanitize = strToSanitize.Replace("û", "u")
        strToSanitize = strToSanitize.Replace("ø", "o")
        strToSanitize = strToSanitize.Replace("î", "i")
        strToSanitize = strToSanitize.Replace("Á", "a")

        strToSanitize = strToSanitize.Replace("å", "a")
        strToSanitize = strToSanitize.Replace("É", "e")
        strToSanitize = strToSanitize.Replace("ô", "o")
        strToSanitize = strToSanitize.Replace("ü", "u")
        strToSanitize = strToSanitize.Replace("š", "s")
