...
Public Class clsCMS
    ...
    Public Function getMovies(ByVal useFilter As Boolean, ByVal searchText As String, ByVal languageid As Integer, ByVal image As Boolean, ByVal synopsis As Boolean, ByVal release As Boolean, _
                                  ByVal cast As Boolean, ByVal verified As Boolean, ByVal exact As Boolean, ByVal awards As Boolean, ByVal updatedby As String, ByVal ImageSynopsisCastMissing As Boolean) As DataTable
        Dim dt As DataTable = Nothing
        Dim obj As New clsExecute
        Try
            Dim strSql As String = ""
            strSql = " select distinct a.* from (SELECT x.rowid ,x.moviename ,convert(VARCHAR, x.rowid) + '_' + replace(replace(replace(convert(VARCHAR(50), x.moviename), ' ', '_'), ':', ''), '''''', '') + '.jpg' filename ,x.starcast ,x.synopsis ,x.longsynopsis" & _
                     " ,x.tmdbimageurl ,x.releaseyear ,x.country ,x.movielangid ,a.Fullname movieLanguage ,x.trivia ,x.awards,x.writer" & _
                     " ,x.director ,x.premierdate ,x.premierchannelid ,isnull(x.active, 0) active ,case when isnull(x.verified, 0)=1 then '1' else '0' end as verified" & _
                     " ,x.genre ,x.genre2 ,x.languageid ,CASE WHEN (SELECT count(*) FROM mst_program WHERE ProgName=x.moviename and progid IN ( Select progid FROM mst_epg" & _
                     " WHERE convert(VARCHAR, progdate, 112) >= convert(VARCHAR, [dbo].[GetLocalDate](), 112) )) > 0 THEN 1 ELSE 0 END AS epg FROM mst_moviesdb x" & _
                     " JOIN mst_language a ON x.movielangid = a.languageid) a" & _
                     " join mst_program b on a.moviename=b.ProgName" & _
                     " join mst_epg e on e.ProgID=b.ProgID where e.ProgTime between '19:00:00' and '22:00:00' and e.duration>70" & _
