...
Public Class highlights
    ...
    Private Sub bindGrid(ByVal paging As Boolean)
        ...
        Dim dt As DataTable = obj.executeSQL("select * from vw_highlights_cms order by channelid,CONVERT(varchar,istdate,112),CONVERT(varchar,isttime,108)", False)
        ...
        For Each row In grdHighLights.Rows
            ...
            If chkHighLight.Checked Then
                ...
                obj.executeSQL("insert into mst_highlights values('" & ChannelId & "','" & ProgName & "','" & Synopsis & "','" & StarCast & "'," & _
                               "'" & ISTDate & "','" & ISTTime & "','" & Genre & "','" & poster & "','" & VADate & "','" & VATime & "',1,'" & progType & "','" & freshRepeat & "','" & FreshAirTimeVA & "')", False)
    ...
    Private Sub ExportExcel()
        ...
        Dim obj As New clsExecute
        Dim dt As DataTable = obj.executeSQL("select Channelid, ProgName,ProgType,vadate [NZ Date], vatime [NZ Time],case when progType='Show' then FreshRepeat else  '' end as FreshRepeat,FreshAirTimeVA,  Synopsis, Genre, Poster from vw_highlights_va order by cast(vadate + ' ' + vatime as datetime),channelid", False)
    ...
    Private Function getXLSFileName() As String
        ...
        Dim strFileName As String = "vaHighlights" & "_" & DateTime.Now.ToString("ddMMMyyyyHHmmss") & ".xls"
