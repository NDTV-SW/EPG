...
Public Class ProgramDescripencyAllLangs
...
#Region "Bind Controls"
    Private Sub bindDdlChannelName()
        Dim obj As New clsExecute
        Dim dt As DataTable = obj.executeSQL("SELECT ChannelId FROM mst_Channel where ChannelID in (select ChannelID from mst_ChannelRegionalName) and channelid not in (select channelID from mst_channel_ppv) and Active='1' and SendEPG='1' and Onair='1' and (AirtelFTP='1' or AirtelMail='1') ORDER BY ChannelId", False)
        ddlChannelName.DataSource = dt
        ddlChannelName.DataBind()

        Dim obj1 As New Logger
        lbEPGExists.Text = obj1.GetEpgDates(ddlChannelName.SelectedValue)
        imgChannel.ImageUrl = "~/Images/Channel/" & ddlChannelName.SelectedValue.Replace(" ", "_") & ".jpg"
        imgChannel.AlternateText = "Channel Name"
    End Sub
    ...
    Private Sub bindDdlLanguage()
        Try
            ...
            Dim obj As New clsExecute
            Dim dt As DataTable = obj.executeSQL("select b.LanguageID LanguageID,b.FullName FullName  from mst_ChannelRegionalName a,mst_language b where channelid='" & ddlChannelName.SelectedValue & "' and a.LanguageId=b.LanguageID and b.active=1", False)
    ...
    Private Sub bindGrdProgramMaster(ByVal paging As Boolean)
        Try
            Dim obj As New clsExecute
            Dim dt As DataTable = obj.executeSQL("sp_epg_validate_synopsis_v1", "ChannelId~Language", "VarChar~Int", ddlChannelName.SelectedValue & "~" & ddlLanguage.SelectedValue, True, False)
    ...
    Private Sub bindGrdSynopsis(ByVal paging As Boolean)
        Try
            Dim obj As New clsExecute
            Dim dt As DataTable = obj.executeSQL("Select * from SYNOPSIS_UPDATE_PENDING where ChannelId='" & ddlChannelName.SelectedValue & "' and fixed=0 and CONVERT(varchar,updatedat,112)=CONVERT(varchar,dbo.GetLocalDate(),112)", False)
    ...
    Private Sub bindGrdXMLGenerated(ByVal paging As Boolean)
        Try
            Dim obj As New clsExecute
            Dim dt As DataTable = obj.executeSQL("select distinct convert(varchar,progdate,106) progdate1,progdate from mst_epg where ChannelId='" & ddlChannelName.SelectedValue & "' and xml_generated=0 and convert(varchar,Progdate,112)>=convert(varchar,dbo.GetLocalDate(),112)order by progdate", False)
    ...
    Private Sub bindGrdGenerateAgain(ByVal paging As Boolean)
        Try
            Dim obj As New clsExecute
            Dim dt As DataTable = obj.executeSQL("select distinct convert(varchar,progdate,106) progdate1,progdate from mst_epg where ChannelId='" & ddlChannelName.SelectedValue & "' and xml_generated=1 and convert(varchar,Progdate,112)>=convert(varchar,dbo.GetLocalDate(),112) and convert(varchar,Progdate,112)<=convert(varchar,dbo.GetLocalDate()+1,112) order by progdate", False)
...
#End Region
    ...
    Private Sub grdProgrammaster_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles grdProgrammaster.RowCommand
        Try
            ...
            Dim txtProgram As TextBox = DirectCast(grdProgrammaster.Rows(Convert.ToInt32(e.CommandArgument)).FindControl("txtProgName"), TextBox)
            Dim txtSynopsis As TextBox = DirectCast(grdProgrammaster.Rows(Convert.ToInt32(e.CommandArgument)).FindControl("txtSynopsis"), TextBox)
            Dim lbEpisodeNo As Label = DirectCast(grdProgrammaster.Rows(Convert.ToInt32(e.CommandArgument)).FindControl("lbEpisodeNo"), Label)
            Dim chkDefaultSynopsis As CheckBox = DirectCast(grdProgrammaster.Rows(Convert.ToInt32(e.CommandArgument)).FindControl("chkDefaultSynopsis"), CheckBox)
            ...
            If e.CommandName.ToLower = "insert" Then
                Dim rowid As String, progid As String, LanguageId As String
                ...
                rowid = Server.HtmlDecode(row.Cells(0).Text.ToString.Trim)
                progid = Server.HtmlDecode(row.Cells(1).Text.ToString.Trim)
                LanguageId = Server.HtmlDecode(row.Cells(2).Text.ToString.Trim)

                If chkDefaultSynopsis.Checked Then

                    Dim obj As New clsExecute
                    Dim dtDefaultSynopsis As DataTable = obj.executeSQL("select synopsis from mst_programregional where progid='" & progid & "' and languageid='" & LanguageId & "' and episodeno='0'", False)
                    If dtDefaultSynopsis.Rows.Count >= 1 Then
                        exec_ProcRegional(progid, Server.HtmlDecode(theProgName.ToString.Trim), dtDefaultSynopsis.Rows(0)("synopsis").ToString.Trim, LanguageId, 0, "A", lbEpisodeNo.Text)
                    Else
                        Logger.LogError("ProgramDescripency", "grdProgrammaster_RowCommand", "Synopsis does not exist for progid: " & progid & ", language: " & LanguageId & ",Episode: 0", User.Identity.Name)
                    End If
                Else
                    exec_ProcRegional(progid, Server.HtmlDecode(theProgName.ToString.Trim), Server.HtmlDecode(theSynopsis.ToString.Trim), LanguageId, 0, "A", lbEpisodeNo.Text)
                End If
                bindGrdProgramMaster(True)
            ElseIf e.CommandName.ToLower = "upd" Then
                ...
                If chkDefaultSynopsis.Checked Then
                    Dim obj As New clsExecute
                    Dim dtDefaultSynopsis As DataTable = obj.executeSQL("select synopsis from mst_programregional where progid='" & progid & "' and languageid='" & LanguageId & "' and episodeno='0'", 0)
                    If dtDefaultSynopsis.Rows.Count >= 1 Then
                        exec_ProcRegional(progid, txtProgram.Text, dtDefaultSynopsis.Rows(0)("synopsis").ToString.Trim, LanguageId, rowid, "U", lbEpisodeNo.Text)
                    Else
                        Logger.LogError("ProgramDescripency", "grdProgrammaster_RowCommand", "Synopsis does not exist for progid: " & progid & ", language: " & LanguageId & ",Episode: 0", User.Identity.Name)
                    End If
                Else
                    exec_ProcRegional(progid, txtProgram.Text, Server.HtmlDecode(theSynopsis.ToString.Trim), LanguageId, rowid, "U", lbEpisodeNo.Text)
                End If
                bindGrdProgramMaster(True)
            End If
                ...
                If chkUpdateSynopsis.Checked Then
                    ...
                    If chkDefaultSynopsis.Checked Then
                        ...
                        If dtDefaultSynopsis.Rows.Count >= 1 Then
                            exec_ProcRegional(progid, txtProgram.Text, dtDefaultSynopsis.Rows(0)("Synopsis").ToString.Trim, LanguageId, rowid, "U", lbEpisodeNo.Text)
                        Else
                            Logger.LogError("ProgramDescripency", "btnUpdateSynopsis_Click", "Synopsis does not exist for progid: " & progid & ", language: " & LanguageId & ",Episode: 0", User.Identity.Name)
                        End If
                    Else
                        exec_ProcRegional(progid, txtProgram.Text, Server.HtmlDecode(theSynopsis.ToString.Trim), LanguageId, rowid, "U", lbEpisodeNo.Text)
                    End If
    ...
    '                Dim dtProgid As DataTable = obj.executeSQL("select top 1 progID from mst_program where channelID='" & ddlChannelName.SelectedValue.ToString.Trim & "' and ltrim(rtrim(progname))=LTRIM(rtrim('" & lbProgname.Text.Replace("'", "''") & "'))", False)
    '                Dim intProgid As Integer = Convert.ToInt32(dtProgid.Rows(0)("Progid").ToString)
    '                'obj.executeSQL("update mst_programregional set synopsis='" & lbNewSynopsis.Text.Replace("'", "''") & "' where languageId=1 and progid=(select top 1 progID from mst_program where channelID='" & ddlChannelName.SelectedValue.ToString.Trim & "' and ltrim(rtrim(progname))=LTRIM(rtrim('" & lbProgname.Text.Replace("'", "''") & "')))", False)
    ...
    '                                    ID & "~" & intProgid & "~" & lbProgname.Text & "~" & lbNewSynopsis.Text & "~0~1~U~" & User.Identity.Name, True, False)
    ...
    Private Sub grdProgrammaster_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles grdProgrammaster.RowDataBound
        Try
            If e.Row.RowType = DataControlRowType.DataRow Then
                ...
                btn.Visible = False
                If Server.HtmlDecode(e.Row.Cells(1).Text).Trim = "" Then
                    btn.Visible = False
    ...
    Private Sub exec_ProcRegional(ByVal ProgId As Integer, ByVal ProgName As String, ByVal synopsis As String, ByVal LanguageID As Integer, ByVal ID As Integer, ByVal action As Char, ByVal vEpisodeNo As Integer)
        Try
            ...
            End If
        Catch ex As Exception
            Logger.LogError("ProgramDiscrepancyAllLangs", action & " exec_ProcRegional", "Message:  Action='" & action & "', Progid='" & ProgId & "',ProgName='" & ProgName & "',Synopsis'""',LanguageId='" & LanguageID & "', RowId='" & ID.ToString & "', ActionUser='" & User.Identity.Name & "' ", User.Identity.Name)
            Logger.LogError("ProgramDiscrepancyAllLangs", action & " exec_ProcRegional", ex.Message.ToString, User.Identity.Name)
        End Try
...
#Region "XML Generation"
    Private Sub btnGenerateXML_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnGenerateXML.Click
        Try
            ...
            CreateXML(ddlChannelName.SelectedValue.ToString.Trim, ddlChannelName.SelectedItem.Text.ToString.Trim, Session("StartDateValue").ToString.Trim, Session("EndDateValue").ToString.Trim)
            If fileError = 1 Then
                myErrorBox("XML File not generated. !")
                Exit Sub
            End If
            CreateXMLRecordDetails(ddlChannelName.SelectedValue.ToString.Trim, ddlChannelName.SelectedItem.Text.ToString.Trim, Session("StartDateValue").ToString.Trim, Session("EndDateValue").ToString.Trim)
    ...
    Private Sub btnGenerateXMLAgain_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnGenerateXMLAgain.Click
        Try
            ...
            CreateXMLAgain(ddlChannelName.SelectedValue.ToString.Trim, ddlChannelName.SelectedItem.Text.ToString.Trim, Session("StartDateValue").ToString.Trim, Session("EndDateValue").ToString.Trim)
            If fileError = 1 Then
                myErrorBox("XML File not generated. !")
                Exit Sub
            End If
    ...
    Private Sub CreateXML(ByVal ChannelId As String, ByVal ChannelName As String, ByVal StartDate As Date, ByVal EndDate As Date)
        Try
            ...
            Dim obj As New clsExecute
            obj.executeSQL("sp_epg_recording", "ChannelId~EPGFromDate~EpgToDate", "VarChar~DateTime~DateTime", ddlChannelName.Text & "~" & StartDate & "~" & EndDate, True, False)
            'obj.executeSQL("fixOverlapping_Channel", "ChannelId~FromDate~ToDate", "VarChar~DateTime~DateTime", ddlChannelName.Text & "~" & StartDate & "~" & EndDate, True, False)

            Dim dt As DataTable = obj.executeSQL("Select [dbo].[FN_XML_EPG_NEW_dev] ('" & ChannelId.ToString.Trim & "','" & StartDate & "','" & EndDate & "')", False)
    ...
    Private Sub CreateXMLAgain(ByVal ChannelId As String, ByVal ChannelName As String, ByVal StartDate As Date, ByVal EndDate As Date)
        Try
            ...
            Dim obj As New clsExecute
            'obj.executeSQL("fixOverlapping_Channel", "ChannelId~FromDate~ToDate", "VarChar~DateTime~DateTime", ddlChannelName.Text & "~" & StartDate & "~" & EndDate, True, False)
            Dim dt As DataTable = obj.executeSQL("Select [dbo].[FN_XML_EPG_NEW_AGAIN_dev] ('" & ChannelId.ToString.Trim & "','" & StartDate & "','" & EndDate & "')", False)
...
#End Region
    ...
    Protected Sub ddlChannelName_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles ddlChannelName.SelectedIndexChanged
        Try
            ...
            lbChannelSelected.Text = ddlChannelName.SelectedItem.Text & " has fresh EPG data (XML not generated yet) for the following dates:"
            lbChannelSelected.Visible = True
        ...
        Catch ex As Exception
            Logger.LogError("Program Discrepancy", "ddlChannelName_SelectedIndexChanged", ex.Message.ToString, User.Identity.Name)
        End Try
    End Sub
